...
class Settings(BaseSettings):
    ...
    # ── Второй фактор для админ-панели ─────────────────────────────
    # Общий код, который владелец и администраторы предъявляют на входе
    # вдобавок к email и паролю: пароль без кода не пускает, утёкший код без
    # пароля -- тоже. Необязателен (пустое значение = вход как раньше), но
    # если задан -- обязателен для ролей owner и admin, и только для них:
    # клиентам и мастерам админ-панель всё равно недоступна.
    #
    # Это именно общий код, а не персональный фактор: он лежит в .env рядом
    # с SECRET_KEY, и тот, кто прочитал .env, знает и его. Настоящую 2FA
    # (TOTP, привязанный к устройству конкретного человека) он не заменяет --
    # он поднимает цену утёкшего или подобранного пароля.
    ...
    # ── Напоминания о записи (за 24ч и за 2ч) ───────────────────────
    # Опрашивается asyncio-лупом в процессе бэкенда (см. app/scheduler.py) —
    # без внешней очереди задач, работает только с одним экземпляром бэкенда.
    reminder_poll_interval_seconds: int = 300
