...
| Настройки | Инлайн-WYSIWYG-редактор контента главной страницы: правки делаются кликом прямо по реальной странице (шапка/hero/блоки «Почему мы»/услуги/мастера/CTA/футер в натуральную величину), тема/шрифты/дизайн hero — в плавающей панели снизу; время работы салона и SEO (заголовок вкладки, описание, favicon) — в модалке «Ещё настройки» той же панели |
...
| Что | Команда | Требует БД? |
|---|---|---|
| Backend unit | `cd backend && pytest -q` | нет (фейковые репозитории) |
| Backend integration | `TEST_DATABASE_URL=... pytest -m integration tests/integration -q` | да, реальный Postgres — см. докстринг `backend/tests/integration/conftest.py` |
| Frontend unit | `cd frontend && npm run test:unit` | нет (Vitest, сторы с замоканным API) |
| Frontend e2e | `cd frontend && npx playwright test` | да — нужен работающий backend на `:8000` с накаченными миграциями (например `docker compose up db backend`); см. докстринг `frontend/tests/e2e/core-flows.spec.js` |

Frontend e2e (Playwright) гоняет 2 сквозных сценария через настоящий браузер поверх настоящего backend + Postgres (первый запуск `/setup` → логин → правки в редакторе настроек; регистрация клиента → бронирование → отмена записи) плюс регрессию доступности (axe-core) на главной/каталоге мастеров/входе — `tests/e2e/pages-a11y.spec.js`, идёт после core-flows.spec.js (нужен уже настроенный сайт). `E2E_API_URL` (по умолчанию `http://localhost:8000/api`) — куда слать сид-запросы (создание услуги/мастера/расписания) в обход UI.
...
Автодеплоя по CI нет (см. «Известные ограничения») — обновления накатываются
руками с сервера, одной командой:

```bash
./scripts/deploy.sh
```

Скрипт делает `git pull` + `docker compose up --build -d --wait` и падает с
ошибкой, если после пересборки backend/frontend/caddy не стали healthy —
не оставляет сломанный деплой молча висеть. Миграции накатываются
автоматически при старте backend-контейнера, отдельного шага не требуется.
...
- Перебор паролей ограничен: временная блокировка входа после 5 неудач за
  15 минут (журнал `login_attempt_log`), запросы сброса пароля — 3/час.
- **Код администратора (`ADMIN_LOGIN_TOKEN`)** — необязательный второй фактор
  для админ-панели. Если переменная задана, владелец и администратор при входе
  предъявляют её значение вдобавок к email и паролю (`POST /api/auth/login`,
  поле `admin_token`); клиентам и мастерам код не нужен — им админ-панель
  закрыта ролью. Форма входа спрашивает его, только когда
  `GET /api/auth/admin-token-required` вернул `true`. Код сверяется
  `secrets.compare_digest`, неверный код даёт тот же 401 и ту же запись в
  `login_attempt_log`, что и неверный пароль, — то есть подбирается не быстрее
  пароля и упирается в ту же блокировку. Пока код включён, владелец и
  администратор не могут войти через VK: в OAuth-потоке код спросить негде,
  и такой вход был бы обходом фактора. Это общий код на всех администраторов,
  лежащий в `.env` рядом с `SECRET_KEY`: он не заменяет персональную 2FA
  (TOTP), а поднимает цену утёкшего или подобранного пароля.

## Известные ограничения

- CI настроен (тесты, линт, интеграционные тесты, сборка — см. `.github/workflows/ci.yml`),
  но CD — нет: обновление на VPS запускается руками, `./scripts/deploy.sh`
  (см. «Деплой на VPS» выше), автозапуска на push/merge не происходит
