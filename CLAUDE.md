...
### Frontend
Vue 3 `<script setup>` + Pinia + Vue Router + Tailwind, no TypeScript. Design tokens (colors, fonts) live in `tailwind.config.js`; the full design rationale (palette, typography, layout patterns per page type, component specs) is in `design-system/сайтама-barbershop/MASTER.md` — read that before changing visual style, it documents *why* choices were made (e.g. why the auto-generated tool suggestion was overridden).
