...
`User.password_hash` is nullable — accounts created via VK ID OAuth have no password. `services/vk_oauth_service.py` implements the redirect flow (`GET /api/auth/vk/login` → id.vk.com authorize with OAuth 2.1 PKCE, state+verifier stashed in short-lived httpOnly cookies scoped to `/api/auth/vk` → `GET /api/auth/vk/callback` exchanges the code, fetches the VK profile, and either links `vk_user_id` onto an existing user matched by email or creates a new `client` via `UserRepository.create_vk_oauth_user()`) then sets the same session cookie as password login and redirects to `settings.frontend_base_url`. `AuthService.login`/`request_password_reset` both guard on `password_hash is None` (OAuth-only accounts can't password-login or reset a password that never existed) using the same constant-time dummy-hash comparison as the "user not found" case, so responses stay indistinguishable. `GET /api/auth/vk/enabled` reports whether `settings.vk_client_id` is configured — the frontend hides the VK button otherwise.
...
- Rubric also expects: search/filter/pagination (`PageParams`/`PageResponse[T]`, used on services/masters/appointments/reviews/admin-users lists), a state machine for appointment status transitions (`ALLOWED_TRANSITIONS`), an admin dashboard with stats (`GET /api/admin/stats`), Swagger docs (already at `/api/docs`). OAuth (VK ID) and real email delivery are now implemented (see `services/vk_oauth_service.py` and `utils/email.py`/`settings.smtp_*` — defaults target a local Mailpit container, `docker-compose.dev.yml`). Still outstanding: responsive-frontend polish, deployment/HTTPS/CI — treat these as the backlog when asked "what's left."
