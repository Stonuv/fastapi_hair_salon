...
// два разных бага.
//
// ПРИЧИНА НАЙДЕНА (2026-08-07, см. память Claude "backend stale-read
// race"): это не протухшее соединение в пуле SQLAlchemy (исходная
// гипотеза была неверна — прямыми замерами через psycopg2 подтверждено,
// что Postgres не видит строку ~13-20мс ПОСЛЕ того, как клиент уже
// получил 201 на создающий запрос). Причина — сам FastAPI: код после
// `yield` в зависимостях (в т.ч. get_db().db.commit()) выполняется при
// закрытии AsyncExitStack. В fastapi/routing.py::get_request_handler
// (venv/lib/python3.14/site-packages/fastapi/routing.py, ~строка 125)
// это выглядит буквально так:
//
//   async with AsyncExitStack() as request_stack:
//       scope["fastapi_inner_astack"] = request_stack
//       ...
//       response = await f(request)      # эндпоинт уже отработал
//   await response(scope, receive, send)  # ответ реально уходит клиенту
//   # request_stack закрывается ЗДЕСЬ — после отправки, не до
//
// т.е. `await response(...)` (реальная отправка байтов клиенту) стоит
// ВНУТРИ `async with AsyncExitStack()`, а не после её закрытия — commit()
// гарантированно выполняется уже после того, как клиент увидел 201/200.
// Это архитектурное свойство FastAPI (проверено на 0.137.0, см.
// backend/venv), а не баг в этом приложении конкретно.
//
// Реальным пользователям это не грозит: единственное место в проде,
// где эти два вызова могли бы оказаться рядом (AdminUsers.vue,
// createMasterProfile) — отдельная кнопка в уже отрисованном списке
// пользователей, то есть между созданием и этим кликом всегда проходит
// как минимум перезагрузка списка (секунды), на порядки больше окна
// гонки. Ловится только автоматизацией без пауз — этим e2e-сидом и
// голым curl-скриптом. Полноценный фикс потребовал бы менять то, как
// коммитится db-сессия для ВСЕХ роутов (общий get_db) — слишком большой
// и рискованный диф прямо перед деплоем ради бага, не видимого живым
// пользователям; ретраи ниже остаются постоянным решением для этого
// сида, а не временной заглушкой.
